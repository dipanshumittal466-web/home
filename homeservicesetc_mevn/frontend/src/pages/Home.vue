  ...
  <div class="p-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div
        ...
        :key="cat._id"
        class="border rounded-lg p-4 shadow hover:shadow-md"
      >
        <img
          v-if="cat.icon"
          :src="cat.icon"
        ...
        <ul class="text-sm mt-2 list-disc ml-4">
          <li v-for="sub in cat.subcategories" :key="sub._id">
            {{ sub.name }}
          </li>
        </ul>
      </div>
...
import api from "../api"; // ✅ api.js se import (ye .env ka baseURL use karega)

const categories = ref([]);

onMounted(async () => {
  try {
    const { data } = await api.get("/categories"); // ✅ ab correct endpoint hit hoga
